...
## 16. FR-18 跨仓库评审新增 MineUI 侧问题（2026-09-20，待 MineUI 处理）

### M1 混合父子路径的 batch 必须生成合法 PATCH（P1，已确认）
- 现状：batch 内先写父路径会覆盖后写的子路径（`add /player` + `replace /player/name` 顺序导致客户端应用失败）。
- 要求（选最简单的正确实现）：batch 按插入顺序保留操作快照，取消跨操作的路径去重优化，
  仍合并为一个网络包；优化留待以后对“顶层子树最终值”做整体压缩。
- 验收：从空客户端状态应用一次混合 batch，序列化后的树与服务端一致。
- MineAudio 说明：本仓库不使用点分键，不受影响；待 MineUI 修复后无需本仓库改动。

### M2 画廊 tab_select 与负载同步（P1，已确认）
- 顶层 Tabs 已改发 `{"tab": n}`，但画廊 `tab_select` 处理器仍读 `index`，点击“搜索/队列”总选中第 0 页；
  文档两种描述并存。
- 要求：同步处理器、示例与文档；如保留旧客户端兼容，只在该 Tabs 动作内明确处理旧负载，
  不得把列表条目 index 误作页签值。
- MineAudio 说明：本仓库页签用普通按钮 + 会话动作，不受影响。
