...
- 界面：`/mineaudio ui` → **曲库** Tab 顶部有 **本地曲库 / 远程曲库** 子页切换；
  本地曲库为**固定 8 个插槽 + 翻页**，支持“试听”（本地直接播放）、“刷新”、上一页/下一页；
  远程曲库为服务端曲目列表。未安装客户端本地 API 时本地曲库为空、动作忽略
  （MineUI 的 `list.items` 只接受 `{state.*}`，不支持本地列表，故用固定插槽 + 本地标量绑定）
